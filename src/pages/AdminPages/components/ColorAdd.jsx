...
import { postColor } from '../../../services/Admin/ProductAPI';

const ColorAdd = ({ getColorData }) => {
    ...
    // Handle Input Changes for Name and Color Code
    const handleChange = (e) => {
        ...
        if (name === 'color_code') {
            setColor(value); // Keep color state in sync for controlled input
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await postColor(values);
            if (response.success) {
                alert('Color added successfully');
                setValues({ name: '', color_code: '' });
                setColor('');
                toggleModal();
                getColorData(); // Refresh parent component's data
            } else {
                    ...
                    <div className="relative p-4 w-full max-w-md max-h-full">
                        <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            ...
                            <div className="flex items-center justify-between p-2 md:p-2 border-b rounded-t dark:border-gray-600">
                                <h3 className="text-md font-semibold text-gray-900 dark:text-white">Create Color</h3>
                                <button
                                    onClick={toggleModal}
